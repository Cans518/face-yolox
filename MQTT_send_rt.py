...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        client.subscribe("nssol/rtb/sensor2", 2)  # 订阅第二个主题
        print("成功订阅主题：nssol/rtb/sensor1 和 nssol/rtb/sensor2")
        print("等待接收消息...")
    else:
        print("连接失败")
        ...
        if topic == "nssol/rtb/sensor1":
            ...
            if frame is not None:
                ...
                if response.status_code == 200:
                    ...
                    print("发送数据失败")
        if topic == "nssol/rtb/sensor2":
            rtsp_url = "rtsp://camera67:nssol2023@nat.reimu.site:39665/ch1/main/av_stream"
            # 使用cv2读取摄像头数据
            frame = capture_camera_frame(rtsp_url)
            if frame is not None:
                http_image,http_name = person.detect_image(frame)
